...
type Step = "details" | "otp" | "success" | "already-registered";
...
// ─────────────────────────────────────────────────────────────────────────────
// OTP Input Component
// REVERT NOTE: restore step === "otp" JSX block and OTP routing in
// handleDetailsSubmit + mount useEffect to re-enable OTP verification.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function RegistrationForm({
  ...
  // Step 2 state — kept intact for easy revert when OTP is re-enabled
  ...
  // ── Cooldown timer — kept for easy revert ─────────────────────────────────
  ...
  // TEMPORARY BYPASS: only the "verified" path is active.
  // The "pending" path is removed — the API now resolves stale records
  // in-place on submit and returns an inviteCode directly.
  //
  // REVERT NOTE: restore the initialStatus === "pending" block below to
  // re-enable OTP auto-resend for pending guests on page load.
  useEffect(() => {
    ...
    // REVERT: restore this block to re-enable OTP routing for pending guests:
    ...
    //         body: JSON.stringify({ registrationId: regId, email: initialEmail }),
    //       });
  ...
  // TEMPORARY BYPASS: on success the API returns { registrationId, inviteCode }
  // and we jump straight to step 3. No OTP step.
  //
  // REVERT NOTE: restore the OTP routing block in the success path and the
  // guest-status fetch inside the 409 handler to re-enable email verification.
  const handleDetailsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Checked first, regardless of res.ok — this response is a 200,
      // not an error, but it's also not a normal successful registration.
      if ((data as { alreadyRegistered?: boolean }).alreadyRegistered) {
      ...
      // Success — API returns { registrationId, inviteCode } for all
      // normal cases (new registration 201, existing verified 200,
      // stale pending resolved 200)
      const { registrationId: regId, inviteCode: code } = data as {
  ...
  // ── Step 2: Submit OTP — kept intact for easy revert ──────────────────────
  ...
  // ── Resend OTP — kept intact for easy revert ───────────────────────────────
  ...
  // ── Resolve WhatsApp links for step 3 ─────────────────────────────────────
  const resolvedWhatsApp = isHybrid
    ? {
        physical: whatsappGroupLinkPhysical || whatsappGroupLink,
        virtual: whatsappGroupLinkVirtual || whatsappGroupLink,
      }
    : { single: whatsappGroupLink };

  const hasWhatsApp = isHybrid
    ? resolvedWhatsApp.physical || resolvedWhatsApp.virtual
    : resolvedWhatsApp.single;
    ...
    <>
      ...
      <div className="rf-root">
        <div className="rf-card">
          ...
          {step === "details" && (
            <>
              <div className="rf-header">
                ...
                {/* REVERT: restore "We'll send a verification code to your email." */}
                <p className="rf-subtitle">
                  No account needed. Fill in your details to get your ticket.
                </p>
              </div>
                  ...
                  <div className="rf-field">
                    ...
                    >
                      {attendanceType === "physical"
                        ? "You'll attend at the physical venue. Venue details will be shared closer to the event."
                        : "You'll join online. A meeting link will be shared closer to the event."}
                    </p>
                  </div>
                )}
          ...
          {/* ── Step 2: OTP verification — kept intact for easy revert ──────── */}
          {step === "otp" && (
            <>
              <div className="rf-header">
                <div className="rf-guest-badge">
          ...
          {step === "already-registered" && (
            <>
              ...
              <div className="rf-header" style={{ textAlign: "center" }}>
                <h1 className="rf-title">You&apos;re Already In!</h1>
                <p className="rf-subtitle">
                  Looks like you already have an account and a ticket for{" "}
                  <strong>{eventTitle}</strong>. We&apos;ve sent the details to
                  your email — log in to view your full ticket.
                </p>
              </div>
            </>
          )}
          ...
          {step === "success" && (
            <>
              ...
              <p className="rf-success-note">
                Present this code at the event entrance. Keep it safe!
              </p>
              ...
              {hasWhatsApp && (
                <div className="rf-whatsapp-section">
                  ...
                  <div className="rf-whatsapp-buttons">
                    {isHybrid ? (
                      <>
                        {resolvedWhatsApp.physical && (
                          <a
                            href={resolvedWhatsApp.physical}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="rf-btn-whatsapp"
                          >
                            📍 Physical Attendees Group
                          </a>
                        )}
                        {resolvedWhatsApp.virtual && (
                          <a
                            href={resolvedWhatsApp.virtual}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="rf-btn-whatsapp"
                          >
                            💻 Virtual Attendees Group
                          </a>
                        )}
                      </>
                    ) : (
