  ...
  const handleDetailsSubmit = async (e: React.FormEvent) => {
   ...
   if (
    ...
    try {
      ...
      if (!res.ok) {
        // 409 now only fires for account-registered emails — show error directly.
        // REVERT NOTE: restore the guest-status fetch + OTP routing here for
        // the case where 409 comes from an existing pending guest record.
        setError(
          isApiError(data)
            ? data.error
            : "Something went wrong. Please try again.",
      ...
      // Success — API returns { registrationId, inviteCode } for all cases
      // (new registration 201, existing verified 200, stale pending resolved 200)
      ...
      // REVERT: restore this OTP routing block instead of the two lines above:
      // setRegistrationId(regId);
      // const [local, domain] = email.split("@");  ← use (data as {email:string}).email
      // setMaskedEmail(
      //   `${local.slice(0, 1)}${"*".repeat(Math.max(local.length - 1, 2))}@${domain}`,
      // );
      // startCooldown(60);
        ...
        <div className="rf-card">
          ...
          {step === "success" && (
            <>
              ...
              {/* REVERT: restore <MigrateCTA /> here and re-add import at top */}
            </>
          )}
        </div>
      </div>
