// GET /api/events/past
// No auth required — for landing page / public past events listings.
// Fetches concluded/past events (eventDate < now)
// Supports ?limit= (max 20) query param.
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Match events that have already concluded (eventDate is in the past)
    const matchStage: Record<string, unknown> = {
      status: "published",
      eventDate: { $lt: now }, // Past events only
    };
