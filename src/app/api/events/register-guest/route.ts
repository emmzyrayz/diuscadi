...
//   3. Validate the ticket type (active, within availability window)
//   4. Guard: reject if email already holds an ACCOUNT registration for this event
//   5. Guard: if email already holds a GUEST registration, return their data
//      directly (bypasses OTP — temporary while email sending limit is resolved)
//   6. Check total event capacity (account + guest combined)
//   7. Check ticket-tier capacity (account + guest combined)
//   8. Validate referral code if provided (check both collections)
//   9. Generate unique inviteCode
//  10. Save guest record with verifiedAt already set (no OTP needed)
//  11. Return { registrationId, inviteCode } for the client to show step 3
//
// TEMPORARY BYPASS: OTP email verification is disabled. verifiedAt is set at
// registration time. Revert by:
//   - Re-adding generateOTP, minutesFromNow imports
//   - Setting emailVerificationCode/emailVerificationExpires in guestDoc
//   - Setting verifiedAt: undefined in guestDoc
//   - Restoring sendGuestVerificationEmail fire-and-forget
//   - Restoring 409 for existingGuest pending/verified cases
//   - Returning { registrationId, email } instead of { registrationId, inviteCode }
// ─────────────────────────────────────────────────────────────────────────────
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── 8. Guard: check if email already has an active GUEST registration ────
    // TEMPORARY BYPASS: instead of returning 409, we surface their existing data
    // directly so they jump straight to step 3 without needing OTP verification.
    const existingGuest = await Collections.guestEventRegistrations(db).findOne(
      {
        email: emailLower,
        eventId: eventObjId,
      ...
      { projection: { _id: 1, verifiedAt: 1, inviteCode: 1 } },
    );
    ...
    if (existingGuest) {
      ...
      // Stale pending record (verifiedAt not set) — verify it in-place
    ...
    // ── 13. Save guest record — verifiedAt set immediately (no OTP) ───────────
    // TEMPORARY BYPASS: emailVerificationCode and emailVerificationExpires are
    // omitted. verifiedAt is set to now so the record is immediately permanent.
    // The TTL index has no target field to act on, so this document won't be
    // auto-deleted.
    const guestDoc: Omit<IGuestEventRegistration, "_id"> = {
      fullName: {
        firstname: firstName!.trim(),
    ...
    // ── 14. Respond — client jumps straight to step 3 ─────────────────────────
    return NextResponse.json(
      {
        registrationId: insertedId.toString(),
        inviteCode,
