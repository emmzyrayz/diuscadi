...
export default function FacultiesSettingsPage() {
  ...
  // Faculties are spread across facultyMap — flatten all
  const items = Object.values(platform.facultyMap).flat();
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="w-14 h-14 rounded-2xl bg-foreground flex items-center justify-center shadow-xl shadow-foreground/20">
            <LuGraduationCap className="w-7 h-7 text-background" />
          </div>
          <div>
        ...
        >
          <LuPlus className="w-5 h-5" /> Add Faculty
        </button>
      </div>
      ...
      <Portal>
        <SettingsCreateModal
          ...
          onConfirm={async (data) => {
            if (!token) return;
            try {
              await platform.createFaculty(data.name as string, token);
        ...
        {editing && (
          <SettingsEditModal
            ...
            onConfirm={async (data) => {
              if (!token) return;
              try {
                await platform.updateFaculty(editing.id, data as never, token);
