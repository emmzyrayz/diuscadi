    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <aside className="lg:col-span-3">
          {/* Mobile Tab Scroller */}
          <div className="lg:hidden flex overflow-x-auto pb-4 gap-2 no-scrollbar border-b border-border">
            {navItems.map((item) => (
              <button
                key={item.id}
          ...
          <nav className="hidden lg:block sticky top-32 space-y-2">
            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-6 px-4">
              Sections
            </p>
            {navItems.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === item.id
                    ? "bg-background text-primary shadow-xl shadow-slate-200/50 border border-border"
