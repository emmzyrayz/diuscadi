...
export const Testimonials = () => {
  return (
    <section className={cn('py-24 w-full', 'bg-foreground', 'text-background', 'overflow-hidden', 'rounded-2xl')}>
      <div className={cn('container', 'mx-auto', 'px-6')}>
        ...
        <div className={cn('text-center', 'max-w-3xl', 'mx-auto', 'mb-16', 'space-y-4')}>
          ...
          <h2 className={cn('text-3xl', 'md:text-5xl', 'font-extrabold', 'tracking-tight')}>
            Trusted Community.{" "}
            <span className="text-secondary">True Connections.</span>
          </h2>
        </div>

        <div className={cn('grid', 'grid-cols-1', 'lg:grid-cols-12', 'gap-12', 'items-start')}>
          ...
          >
            <div className={cn('group', 'relative', 'aspect-video', 'rounded-[2rem]', 'overflow-hidden', 'border-4', 'border-background/10', 'shadow-2xl')}>
              <Image
                src={networking} // Replace with networking-diuscadi.webp
                alt="Networking at DIUSCADI"
                fill
                className={cn('object-cover', 'group-hover:scale-105', 'transition-transform', 'duration-700')}
              />
              <div className={cn('absolute', 'inset-0', 'bg-black/40', 'flex', 'items-center', 'justify-center')}>
                <button className={cn('w-20', 'h-20', 'bg-primary', 'rounded-full', 'flex', 'items-center', 'justify-center', 'shadow-2xl', 'group-hover:scale-110', 'transition-transform')}>
                  <Play className={cn('w-8', 'h-8', 'text-background', 'fill-background', 'ml-1')} />
                </button>
              </div>
              {/* Glass Info Tag */}
              <div className={cn('absolute', 'bottom-4', 'left-4', 'right-4', 'p-4', 'rounded-xl', 'bg-background/10', 'backdrop-blur-md', 'border', 'border-background/20')}>
                <p className={cn('text-sm', 'font-medium')}>
                  Watch the #LASCDSSHighlights
                </p>
              </div>
            </div>
            <div className={cn('p-6', 'bg-background/5', 'rounded-2xl', 'border', 'border-background/10')}>
              <div className={cn('flex', 'gap-1', 'text-secondary', 'mb-3')}>
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={cn('w-4', 'h-4', 'fill-secondary')} />
                ))}
              </div>
          ...
          <div className={cn('lg:col-span-7', 'grid', 'grid-cols-1', 'md:grid-cols-2', 'gap-6', 'max-h-[700px]')}>
            {TESTIMONIALS.map((t, idx) => (
              <motion.div
                ...
                className={cn('p-8', 'rounded-3xl', 'bg-background/5', 'border', 'border-background/10', 'flex', 'flex-col', 'justify-between', 'hover:bg-background/10', 'transition-colors', 'group')}
              >
                <div>
                  <Quote className={cn('w-8', 'h-8', 'text-primary/40', 'mb-4', 'group-hover:text-primary', 'transition-colors')} />
                  <p className={cn('text-slate-300', 'text-sm', 'leading-relaxed', 'mb-6')}>
                    {t.review}
                  </p>
                </div>

                <div className={cn('flex', 'items-center', 'gap-4', 'pt-6', 'border-t', 'border-background/10')}>
                  <div className={cn('relative', 'w-12', 'h-12', 'rounded-full', 'overflow-hidden', 'border-2', 'border-primary/20')}>
                    <Image
                      src={t.image}
                      alt={t.name}
                      fill
                  ...
                  <div>
                    <h5 className={cn('font-bold', 'text-background', 'text-sm')}>{t.name}</h5>
                    <p className={cn('text-xs', 'text-primary', 'font-medium')}>{t.role}</p>
                  </div>
                </div>
              </motion.div>
            ))}
