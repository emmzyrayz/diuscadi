      ...
      {isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={onClose} className="absolute inset-0 bg-foreground/60 backdrop-blur-sm" />
          ...
          >
            <button onClick={onClose} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-foreground transition-colors cursor-pointer">
                ...
                <div key={f.key} className="space-y-2">
                  ...
                  {f.type === "select" ? (
                    <select value={String(values[f.key] ?? "")} onChange={(e) => set(f.key, e.target.value)}
                      className={cn("w-full bg-muted border border-border rounded-2xl p-4 text-sm font-bold outline-none focus:border-primary transition-all")}>
                    ...
                    <input type={f.type} value={String(values[f.key] ?? "")} onChange={(e) => set(f.key, f.type === "number" ? parseFloat(e.target.value) || 0 : e.target.value)}
