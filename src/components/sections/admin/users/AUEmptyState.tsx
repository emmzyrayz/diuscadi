...
export const AdminUsersEmptyState: React.FC<AdminUsersEmptyStateProps> = ({
  ...
  return (
    <motion.div
      ...
      className="w-full py-32 bg-background border-2 border-dashed border-border rounded-[3rem] flex flex-col items-center justify-center text-center px-8"
    >
      ...
      <div className="relative mb-8">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-muted/50 rounded-full blur-3xl -z-10" />
      </div>
      ...
      {isSearchActive && (
        <button
          ...
          className="mt-10 px-8 py-4 bg-foreground text-background rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-foreground transition-all shadow-xl shadow-foreground/10 active:scale-95"
        >
          Reset All Filters
        </button>
      )}
