...
export const AdminUserRow: React.FC<AdminUserRowProps> = ({
  ...
  return (
    <tr
      className={`group transition-all border-b border-slate-50 last:border-0 ${isSelected ? "bg-primary/5" : "hover:bg-muted/80"} ${isBanned ? "opacity-75" : ""}`}
    >
      {/* 1. UserCheckbox */}
      ...
      <td className="px-6 py-5 min-w-[250px]">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="w-11 h-11 rounded-full bg-slate-200 overflow-hidden border-2 border-background shadow-sm shrink-0">
              <Image
                height={300}
                width={500}
                src={user.avatar}
          ...
          <div className="flex flex-col">
            <span
              className={`text-sm font-black tracking-tight ${isBanned ? "text-muted-foreground line-through" : "text-foreground group-hover:text-primary transition-colors"}`}
            >
              {user.name}
            </span>
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <button
                ...
                className="text-slate-300 hover:text-foreground transition-colors"
              >
                {copiedField === "email" ? (
      ...
      <td className="px-6 py-5">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-1.5">
            <LuHash className="w-3 h-3 text-muted-foreground" />
            <span className="text-[10px] font-black text-foreground font-mono tracking-widest">
              {user.inviteCode}
            </span>
            <button
      ...
      <td className="px-8 py-5 text-right relative">
        <button
          ...
          className="p-2 hover:bg-background border border-transparent hover:border-border rounded-lg text-muted-foreground hover:text-foreground transition-all shadow-sm"
        >
...
const Badge = ({ type, value }: { type: string; value: string }) => {
  ...
  if (type === "registration") {
    colorStyles = "text-muted text-slate-600 border-border"; // Neutral for Reg Type
